...
// Category order for the tech capabilities block mirrors the /tech index.
const TECH_CATEGORY_ORDER = ['Design', 'Build', 'Technical', 'Production']
...
const FAQ_ITEMS: FaqItem[] = [
  ...
  },
  {
    ...
    answer:
      'Yes. LED video wall rentals, sound equipment rentals, DJ equipment rentals, and backline and stage rental are all available standalone, with experienced technicians and the same touring-grade inventory that powers our full productions.',
  },
  {
...
export default async function ServicesPage({ preview = false }: { preview?: boolean }) {
  ...
  // Tech capabilities block (P3-04 service mesh): the 31 /tech pages grouped
  // by category, cross-linked from the services silo.
  const techServices = services.filter((service) => service.pageType === 'tech')
  const techGroups = TECH_CATEGORY_ORDER.map((label) => ({
    label,
    items: techServices.filter((service) => service.category === label),
  })).filter((group) => group.items.length > 0)
  const knownTechCategories = new Set(TECH_CATEGORY_ORDER)
  const extraTech = techServices.filter((service) => !knownTechCategories.has(service.category))
  if (extraTech.length > 0) techGroups.push({ label: 'More', items: extraTech })
  ...
  return (
    <>
      ...
      {/* Tech capabilities catalog: the /tech silo cross-linked from here,
          grouped by the same categories as the /tech index */}
      <section className="section border-b border-white/10">
        <div className="max-w-[1400px] mx-auto">
          <div className="flex flex-wrap items-end justify-between gap-6 mb-10">
            <h2 className="heading-lg">The Tech Behind Every Service</h2>
            <Link
              href="/tech"
              className="text-sm font-semibold uppercase tracking-[0.18em] text-white transition-colors hover:text-[var(--accent)]"
            >
              All Tech Capabilities →
            </Link>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
