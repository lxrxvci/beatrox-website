 ...
 * pages (the two were near-verbatim duplicates). HUD card formatting:
 * trust → grid of .hud-card ✓ cards, process → numbered cards with a
 * .hud-index accent chip, default blocks with items → .hud-card bullet
 * cards (content stays prose), faq → divided list in a .hud-card panel.
 * Section headings render as bracketed .hud-label mono labels. Every
 * EditableText wrapper and `body.N.*` field path matches the previous
 * inline JSX exactly.
 */
        ...
        <article key={i} className="space-y-8">
          <div>
            {block.type === 'trust' && (
              <div>
                {block.heading && (
                  <h2 className="hud-label mb-6">{block.heading}</h2>
                )}
            ...
            {block.type === 'faq' && (
              <div>
                ...
                <div className="hud-card p-6 md:p-8 divide-y divide-white/10">
                  <span className="hud-corners" aria-hidden="true" />
